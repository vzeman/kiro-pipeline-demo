...
const TIER_CHECKS: Record<number, string[]> = {
  ...
  2: ['lint', 'type-check', 'test', 'build', 'harness-smoke'],
  3: ['lint', 'type-check', 'test', 'build', 'structural-tests', 'manual-approval', 'harness-smoke'],
};
...
function loadConfig(repoRoot: string): HarnessRiskConfig {
  ...
  if (!existsSync(configPath)) {
    console.log('::error::harness.config.json not found');
    process.exit(1);
  }

  try {
    const raw = readFileSync(configPath, 'utf-8');
    return JSON.parse(raw) as HarnessRiskConfig;
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    console.log(`::error::Failed to parse harness.config.json: ${msg}`);
    process.exit(1);
  }
}
