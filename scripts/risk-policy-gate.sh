...
classify_file() {
  ...
  case "$file" in
    src/index.ts|package.json|tsconfig.json|eslint.config.js) echo 3; return ;;
  esac

  # Tier 2: Source code and tests
  case "$file" in
    src/**/*.ts|tests/**/*.ts|vitest.config.ts) echo 2; return ;;
  esac

  # Tier 1: Documentation
  case "$file" in
    *.md|docs/*|LICENSE) echo 1; return ;;
  esac

  # Default: medium scrutiny
  echo 2
}
...
compute_required_checks() {
  case "$MAX_TIER" in
    1)
      ...
      ;;
    2)
      REQUIRED_CHECKS=("lint" "type-check" "test" "build" "harness-smoke")
      ;;
    3)
      REQUIRED_CHECKS=("lint" "type-check" "test" "build" "structural-tests" "manual-approval" "harness-smoke")
      ;;
    *)
